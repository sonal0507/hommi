...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Attach form handlers to prevent any issues
    attachFormHandlers();
});
...
function attachFormHandlers() {
    ...
    const forms = document.querySelectorAll('form.project-form');
    forms.forEach((form, index) => {
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            const stepNumber = index + 1;
            console.log(`Form step ${stepNumber} submitted`);
    ...
    // Also attach direct button handlers as backup
    setTimeout(() => {
...
function attachDirectButtonHandlers() {
    // Backup solution: Direct button click handlers
    const nextButtons = document.querySelectorAll('button[type="submit"].btn-primary');
    nextButtons.forEach((button, index) => {
        if (button.textContent.trim() === 'Next') {
            button.addEventListener('click', function(e) {
                ...
                // Determine which step we're on based on the screen
                const activeScreen = document.querySelector('.screen.active');
                if (activeScreen) {
                    const screenId = activeScreen.id;
                    console.log('Direct button click on screen:', screenId);
                    
                    switch(screenId) {
                        case 'home-details-form':
                            handleStep1Safe(e);
                            break;
                        case 'home-details-step2':
                            handleStep2Safe(e);
                            break;
                        case 'home-details-step3':
                            handleStep3Safe(e);
                            break;
                        case 'home-details-step4':
                            handleStep4Safe(e);
                            break;
                    }
                }
...
// Bulletproof form handlers
function handleStep1Safe(event) {
    event.preventDefault();
    console.log('Step 1 handler called');
...
function handleStep2Safe(event) {
    event.preventDefault();
    console.log('Step 2 handler called');
    ...
    console.log('Form values:', { homeType, bedrooms, squareFeet, bathrooms, location });
...
function handleStep3Safe(event) {
    event.preventDefault();
...
function handleStep4Safe(event) {
    event.preventDefault();
    console.log('Step 4 handler called');
    ...
    if (budget && budget.trim()) {
        ...
        saveProjectData();
        console.log('Project setup complete, navigating to dashboard');
...
// Simple direct navigation functions - NO FORMS, JUST CLICKS
function proceedToStep3() {
    console.log('Direct navigation to Step 3');
    
    // Get values if they exist, or use defaults
    const homeType = document.getElementById('home-type-step2')?.value || 'villa';
    const bedrooms = document.getElementById('bedrooms-step2')?.value || '2';
    const squareFeet = document.getElementById('square-feet-step2')?.value || '1000-1500';
    const bathrooms = document.getElementById('bathrooms-step2')?.value || '1';
    const location = document.getElementById('home-location-step2')?.value || 'Pune';
    ...
    showScreen('home-details-step3');
    showToast('✅ Moving to Step 3');
}

function proceedToStep4() {
    console.log('Direct navigation to Step 4');
    ...
    saveProjectData();
    showScreen('home-details-step4');
    showToast('✅ Moving to Step 4');
}

function proceedToDashboard() {
    console.log('Direct navigation to Dashboard');
    
    const budget = document.getElementById('budget-step4')?.value || '₹25,00,000';
    if (!currentProject) currentProject = {};
...
function proceedToStep2() {
    ...
    const homeName = document.getElementById('home-name-step1')?.value || 'My Dream Home';
    if (!currentProject) currentProject = {};
    currentProject.homeName = homeName;
    saveProjectData();
    showScreen('home-details-step2');
    showToast('✅ Moving to Step 2');
}

// IMMEDIATE FIX FUNCTION - Call this from console if still stuck
function forceNextStep() {
    const activeScreen = document.querySelector('.screen.active');
    if (activeScreen) {
        const screenId = activeScreen.id;
        console.log('Force navigation from:', screenId);
        ...
        switch(screenId) {
            ...
            case 'home-details-step4':
                ...
                break;
            default:
                showToast('Navigation completed or unknown screen');
        }
    }
}
...
window.handleStep1 = handleStep1Safe;
window.handleStep2 = handleStep2Safe;
window.handleStep3 = handleStep3Safe;
window.handleStep4 = handleStep4Safe;
